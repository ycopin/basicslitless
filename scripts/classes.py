# Time-stamp: <2021-06-08 12:02:08 ycopin>

import numpy as np
from configparser import ConfigParser
from astropy.wcs import WCS

from geometrysl import Order, Rotation_Around
...
class configuration :
   ...
   def __init__(self, fichier = "default_conf"):
      ...
      fichier must be the path to a .ini file, containing the requiered
...
class star_class :
   ...
   def __init__(self, ra, dec, mag,config):
      """
      Sets a star object from the given informations. Set all parameters to 'd'
      for a default star (HD107696 here)
...
def list_stars(table,config, maglim, seeing, orders):
   ...
   ra_colname = [ colname for colname in table.colnames if 'ra' in colname.lower() ][0]
   dec_colname = [ colname for colname in table.colnames if 'dec' in colname.lower() ][0]
   mag_colname = [ colname for colname in table.colnames if 'flux' in colname.lower() ][0]
