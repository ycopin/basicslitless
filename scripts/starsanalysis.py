...
def total_overlap(cstar, residues, angle):
   ...
   Parameters
   ----------
   cstars : star_class object
      The central star
   residues :list of stars
      The list of residual stars, the ones that could contaminate the central one
   angle : float
      angle of dispersion of the PSF on the captor in radian, an angle of zero corresponds to a dispersion along the X axis.
   
   Returns
   -------
   out : the percentage of overlapping on the central star spectrum caused by its surroundings."""
   try:
      residues[0]
   except:
      residues = [residues]
   ...
   M = Rotation_Around(Pos, [cx,cy], -angle)
